...
//         const now = new Date(); // Get current date in UTC

//         // Get the current active streak window
//         const streakWindow = await StreakWindow.findOne().sort({ createdAt: -1 });
//         if (!streakWindow) {
//             return res.status(404).json({ error: 'No active streak window found' });
//         }
...
//         // Check if the current time is within the streak window
//         if (now >= startTime && now <= endTime) {
//             if (user.lastPostDate && user.lastPostDate >= startTime && user.lastPostDate <= endTime) {
//                 newStreakCount = user.streakCount + 1; // Continue streak
//             } else {
//                 newStreakCount = 1; // Start new streak (First post in this window)
//             }
//         } else {
//             // If they missed a streak window, reset to 0
//             newStreakCount = 0;
//         }
